...
#define SDSPI_CH422_TRANSACTION_COUNT 4
#define SDSPI_CH422_MOSI_IDLE_VAL     0xff    //!< Data value which causes MOSI to stay high
#define GPIO_UNUSED 0xff                //!< Flag indicating that CD/WP is unused
/// Size of the buffer returned by get_block_buf
#define SDSPI_CH422_BLOCK_BUF_SIZE    (SDSPI_CH422_MAX_DATA_LEN + 4)
/// Maximum number of dummy bytes between the request and response (minimum is 1)
#define SDSPI_CH422_RESPONSE_MAX_DELAY  8
#define SDSPI_CH422_MAX_DATA_LEN         SDSPI_MAX_DATA_LEN
#define SDSPI_CH422_CMD_R1_SIZE          SDSPI_CMD_R1_SIZE
#define SDSPI_CH422_CMD_R2_SIZE          SDSPI_CMD_R2_SIZE
#define SDSPI_CH422_CMD_R3_SIZE          SDSPI_CMD_R3_SIZE
#define SDSPI_CH422_CMD_R4_SIZE          SDSPI_CMD_R4_SIZE
#define SDSPI_CH422_CMD_R5_SIZE          SDSPI_CMD_R5_SIZE
#define SDSPI_CH422_CMD_R7_SIZE          SDSPI_CMD_R7_SIZE
#define SDSPI_CH422_NCR_MAX_SIZE         SDSPI_NCR_MAX_SIZE
#define SDSPI_CH422_NCR_MIN_SIZE         SDSPI_NCR_MIN_SIZE
#define SDSPI_CH422_NCR_DELAY_US         SDSPI_NCR_DELAY_US
#define SDSPI_CH422_MMC_RESPONSE_MAX_DELAY SDSPI_MMC_RESPONSE_MAX_DELAY
#define SDSPI_CH422_IO_INT_POLLING_DELAY SDSPI_IO_INT_POLLING_DELAY

/**
 * @brief Structure containing run time configuration for a single SD slot
 *
 * The slot info is referenced to by an sdspi_ch422_dev_handle_t (alias int). The handle may be the raw
...
typedef struct {
    ...
    ch422_handle_t *expander;   //!< Optional CH422 expander controlling CS
    ch422_pin_t expander_cs_pin; //!< Expander pin index for CS control
    bool expander_cs_active_low; //!< True if CS is active low on expander
    ...
    /// allocated on demand, SDSPI_CH422_BLOCK_BUF_SIZE bytes long. May be zero.
    uint8_t* block_buf;
    /// semaphore of gpio interrupt
    SemaphoreHandle_t   semphr_int;
} sdspi_ch422_slot_info_t;

// Reserved for old API to be back-compatible
static sdspi_ch422_slot_info_t *s_ch422_slots[SOC_SPI_PERIPH_NUM] = {};
static const char *TAG = "sdspi_ch422";
...
static esp_err_t start_command_read_blocks(sdspi_ch422_slot_info_t *slot, sdspi_hw_cmd_t *cmd,
        uint8_t *data, uint32_t rx_length, bool need_stop_command);

static esp_err_t start_command_write_blocks(sdspi_ch422_slot_info_t *slot, sdspi_hw_cmd_t *cmd,
        const uint8_t *data, uint32_t tx_length, bool multi_block, bool stop_trans);

static esp_err_t start_command_default(sdspi_ch422_slot_info_t *slot, int flags, sdspi_hw_cmd_t *cmd);

static esp_err_t shift_cmd_response(sdspi_hw_cmd_t *cmd, int sent_bytes);

static esp_err_t poll_busy(sdspi_ch422_slot_info_t *slot, int timeout_ms, bool polling);

/// A few helper functions

/// Map handle to pointer of slot information
static sdspi_ch422_slot_info_t* get_slot_info(sdspi_ch422_dev_handle_t handle)
{
    if ((uint32_t) handle < SOC_SPI_PERIPH_NUM) {
        return s_ch422_slots[handle];
    } else {
        return (sdspi_ch422_slot_info_t *) handle;
    }
}

/// Store slot information (if possible) and return corresponding handle
static sdspi_ch422_dev_handle_t store_slot_info(sdspi_ch422_slot_info_t *slot)
{
    /*
     ...
     */
    if (s_ch422_slots[slot->host_id] == NULL) {
        s_ch422_slots[slot->host_id] = slot;
        return slot->host_id;
    } else {
        return (sdspi_ch422_dev_handle_t)slot;
    }
}

/// Get the slot info for a specific handle, and remove the local reference (if exist).
static sdspi_ch422_slot_info_t* remove_slot_info(sdspi_ch422_dev_handle_t handle)
{
    if ((uint32_t) handle < SOC_SPI_PERIPH_NUM) {
        sdspi_ch422_slot_info_t* slot = s_ch422_slots[handle];
        s_ch422_slots[handle] = NULL;
        return slot;
    } else {
        return (sdspi_ch422_slot_info_t *) handle;
    }
...
static void cs_set(sdspi_ch422_slot_info_t *slot, bool level)
{
    if (slot->expander) {
        bool actual = slot->expander_cs_active_low ? !level : level;
        esp_err_t err = ch422_set_pin_level(slot->expander, slot->expander_cs_pin, actual);
        if (err != ESP_OK) {
            ESP_LOGE(TAG, "CH422 CS level set failed (%s)", esp_err_to_name(err));
        }
    } else if (slot->gpio_cs != GPIO_UNUSED) {
        gpio_set_level(slot->gpio_cs, level ? 1 : 0);
    }
}

/// Set CS high for given slot
static void cs_high(sdspi_ch422_slot_info_t *slot)
{
    cs_set(slot, true);
}

/// Set CS low for given slot
static void cs_low(sdspi_ch422_slot_info_t *slot)
{
    cs_set(slot, false);
}

/// Return true if WP pin is configured and is low
static bool card_write_protected(sdspi_ch422_slot_info_t *slot)
{
...
static bool card_missing(sdspi_ch422_slot_info_t *slot)
{
...
static esp_err_t get_block_buf(sdspi_ch422_slot_info_t *slot, uint8_t **out_buf)
{
    if (slot->block_buf == NULL) {
        slot->block_buf = heap_caps_malloc(SDSPI_CH422_BLOCK_BUF_SIZE, MALLOC_CAP_DMA);
        if (slot->block_buf == NULL) {
...
static void release_bus(sdspi_ch422_slot_info_t *slot)
{
    spi_transaction_t t = {
...
static void go_idle_clockout(sdspi_ch422_slot_info_t *slot)
{
 ...
 */
static esp_err_t configure_spi_dev(sdspi_ch422_slot_info_t *slot, int clock_speed_hz)
{
    if (slot->spi_handle) {
        // Reinitializing
    ...
    spi_device_interface_config_t devcfg = {
        ...
        .queue_size = SDSPI_CH422_TRANSACTION_COUNT,
    };
...
static esp_err_t deinit_slot(sdspi_ch422_slot_info_t *slot)
{
...
esp_err_t sdspi_ch422_host_remove_device(sdspi_ch422_dev_handle_t handle)
{
    ...
    sdspi_ch422_slot_info_t* slot = remove_slot_info(handle);
    if (slot == NULL) {
...
{
    for (size_t i = 0; i < sizeof(s_ch422_slots)/sizeof(s_ch422_slots[0]); ++i) {
        sdspi_ch422_slot_info_t* slot = remove_slot_info(i);
...
{
    sdspi_ch422_slot_info_t *slot = get_slot_info(handle);
    if (slot == NULL) {
        return ESP_ERR_INVALID_ARG;
    }
...
static void gpio_intr(void* arg)
{
    ...
    sdspi_ch422_slot_info_t* slot = (sdspi_ch422_slot_info_t*)arg;
...
esp_err_t sdspi_ch422_host_init_device(const sdspi_ch422_device_config_t* slot_config, sdspi_ch422_dev_handle_t* out_handle)
{
    const sdspi_device_config_t *base = &slot_config->base;
    ESP_LOGD(TAG, "%s: SPI%d cs=%d cd=%d wp=%d (expander=%p)",
             __func__, base->host_id + 1, base->gpio_cs,
             base->gpio_cd, base->gpio_wp, (void *)slot_config->expander);

    sdspi_ch422_slot_info_t* slot = (sdspi_ch422_slot_info_t*)calloc(1, sizeof(sdspi_ch422_slot_info_t));
    if (slot == NULL) {
        return ESP_ERR_NO_MEM;
    }
    *slot = (sdspi_ch422_slot_info_t) {
        .host_id = base->host_id,
        .gpio_cs = base->gpio_cs,
        .gpio_cd = GPIO_UNUSED,
        .gpio_wp = GPIO_UNUSED,
        .gpio_int = GPIO_UNUSED,
        .expander = slot_config->expander,
        .expander_cs_pin = slot_config->expander_cs_pin,
        .expander_cs_active_low = slot_config->expander_cs_active_low,
    };
    ...
    if (slot->expander) {
        ...
        if (slot_config->expander_cs_pin > CH422_PIN_EXT1) {
            ret = ESP_ERR_INVALID_ARG;
            ESP_LOGE(TAG, "Invalid CH422 CS pin %d", slot_config->expander_cs_pin);
            goto cleanup;
        }
        cs_high(slot);
    } else {
        ...
        if (base->gpio_cs != SDSPI_SLOT_NO_CS) {
            io_conf.pin_bit_mask = 1ULL << base->gpio_cs;
            slot->gpio_cs = base->gpio_cs;
        } else {
            slot->gpio_cs = GPIO_UNUSED;
        }
    ...
    if (base->gpio_cd != SDSPI_SLOT_NO_CD) {
        io_conf.pin_bit_mask |= (1ULL << base->gpio_cd);
        slot->gpio_cd = base->gpio_cd;
    } else {
        slot->gpio_cd = GPIO_UNUSED;
    }

    if (base->gpio_wp != SDSPI_SLOT_NO_WP) {
        io_conf.pin_bit_mask |= (1ULL << base->gpio_wp);
        slot->gpio_wp = base->gpio_wp;
    } else {
    ...
    if (base->gpio_int != SDSPI_SLOT_NO_INT) {
        slot->gpio_int = base->gpio_int;
        io_conf = (gpio_config_t) {
            .intr_type = GPIO_INTR_LOW_LEVEL,
            .mode = GPIO_MODE_INPUT,
            .pull_up_en = true,
            .pin_bit_mask = (1ULL << base->gpio_int),
        };
        ret = gpio_config(&io_conf);
        if (ret != ESP_OK) {
...
esp_err_t sdspi_ch422_host_start_command(sdspi_ch422_dev_handle_t handle, sdspi_hw_cmd_t *cmd, void *data,
...
static esp_err_t poll_busy(sdspi_ch422_slot_info_t *slot, int timeout_ms, bool polling)
{
    ...
    int nonzero_count = 0;
    do {
        t_rx = SDSPI_CH422_MOSI_IDLE_VAL;
        t.rx_data[0] = 0;
        if (polling) {
...
static esp_err_t poll_data_token(sdspi_ch422_slot_info_t *slot, uint8_t *extra_ptr, size_t *extra_size, int timeout_ms)
{
    uint8_t t_rx[8];
    spi_transaction_t t = {
        .tx_buffer = &t_rx,
    ...
    int64_t t_end = esp_timer_get_time() + timeout_ms * 1000;
    do {
        memset(t_rx, SDSPI_CH422_MOSI_IDLE_VAL, sizeof(t_rx));
        ret = spi_device_polling_transmit(slot->spi_handle, &t);
        if (ret != ESP_OK) {
            return ret;
        }
 ...
 * 1. send command + receive r1 response (SDSPI_CH422_CMD_R1_SIZE bytes total)
 * 2. keep receiving bytes until TOKEN_BLOCK_START is encountered (this may
 *    take a while, depending on card's read speed)
 * 3. receive up to SDSPI_CH422_MAX_DATA_LEN = 512 bytes of actual data
        ...
        uint8_t *data, uint32_t rx_length, bool need_stop_command)
{
    spi_transaction_t t_command = {
        .length = (SDSPI_CH422_CMD_R1_SIZE + SDSPI_CH422_RESPONSE_MAX_DELAY) * 8,
        .tx_buffer = cmd,
        .rx_buffer = cmd,
    };
    ...
    size_t pre_scan_data_size = SDSPI_CH422_RESPONSE_MAX_DELAY;
    uint8_t* pre_scan_data_ptr = cmd_u8 + SDSPI_CH422_CMD_R1_SIZE;
    ...
    while (rx_length > 0) {
        ...
        if (need_poll) {
            ...
            ret = poll_data_token(slot, cmd_u8 + SDSPI_CH422_CMD_R1_SIZE, &extra_data_size, cmd->timeout_ms);
            if (ret != ESP_OK) {
                return ret;
            }
            if (extra_data_size) {
                extra_data_ptr = cmd_u8 + SDSPI_CH422_CMD_R1_SIZE;
            }
        }

        // Arrange RX buffer
        size_t will_receive = MIN(rx_length, SDSPI_CH422_MAX_DATA_LEN) - extra_data_size;
        uint8_t* rx_data;
        ...
        // receive actual data
        const size_t receive_extra_bytes = (rx_length > SDSPI_CH422_MAX_DATA_LEN) ? 4 : 2;
        ...
        const uint8_t *data, uint32_t tx_length, bool multi_block, bool stop_trans)
{
    ...
    const int send_bytes = SDSPI_CH422_CMD_R5_SIZE+SDSPI_CH422_NCR_MAX_SIZE-SDSPI_CH422_NCR_MIN_SIZE;
    ...
    while (tx_length > 0) {
        ...
        size_t will_send = MIN(tx_length, SDSPI_CH422_MAX_DATA_LEN);
